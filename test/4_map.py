...
def merge_gff3(file_path):
    ...
    merged_file_path = '4_merged.gff3'
...
preds_file = DIR + 'rh_outputs/'+txid+'/_operons.txt' 
...
# --------------h.pylori----------------
def find_name_hp(row):
    start = row['Start']
    end = row['Stop']
    # genes between start and end are in the operon
    genes = map[(map['start'] >= start) & (map['end'] <= end)].name
    row['new_Genes'] = ','.join(genes.values)
    return row
if txid == 'txid85962':
    preds = preds.apply(find_name_hp, axis=1)
# ---------------------------------------
...
preds.to_csv(DIR + 'rh_outputs/operons.txt', sep='\t', index=False, header=False)
